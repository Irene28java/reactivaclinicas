...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        db.run(
            ...
            [clinic_name.trim(), email.toLowerCase().trim(), hash, "basic", new Date().toISOString()],
            function(err) {
                if (err) {
    ...
    db.get(
        ...
        async (err, user) => {
            ...
            const token = jwt.sign(
                { id: user.id, clinic_name: user.clinic_name, plan: user.plan },
...
// ─────────────────────────────────────────────
// GET /api/auth/me  — verificar token y obtener perfil
// ─────────────────────────────────────────────
router.get('/me', require('../middleware/auth'), (req, res) => {
    db.get(`SELECT id,clinic_name,email,plan,created_at FROM users WHERE id=?`,
        [req.user.id],
        (err, user) => {
